...
function roleBadge(role: string | null | undefined): string {
    const value = role || '—';
    return `<span class="badge bg-primary-subtle text-primary">${escapeHtml(value)}</span>`;
}
...
function fillSelect(select: HTMLSelectElement, options: Array<{ value: string; label: string }>, includeBlank = false): void {
    select.innerHTML = '';
    if (includeBlank) {
                ...
                filters: [
                    {
                        ...
                        options: [
                            ...
                            ...meta.roles.map((role) => ({ value: role, label: role })),
                        ],
                    },
                    {
                        id: 'is_active',
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        render: (_data, _type, row) =>
                            escapeHtml(row.department ? `${row.department.code}` : '—'),
                    },
                    {
