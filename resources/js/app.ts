...
async function bootAll(): Promise<void> {
    initAppShell();
    ...
    if (document.querySelector('[data-privilege-flowchart]')) {
        await boot('privilege flowchart', async () => {
            const { initPrivilegeFlowchart } = await import('./modules/documentation/privilege-flowchart');
            initPrivilegeFlowchart();
        });
    }
}
